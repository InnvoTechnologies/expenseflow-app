    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(onPressed: _createOrg, icon: const Icon(Icons.add), label: const Text('Create Organization')),
          const SizedBox(height: 24),
          Text('Members', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          ...DummyData.members.where((m) => m.organizationId == DummyData.organizations[selectedIndex].id).map((m) => Card(
                child: ListTile(
                  leading: const Icon(Icons.person),
                  title: Text(m.userId),
                  subtitle: Text('${m.role} • ${m.status}'),
                  trailing: PopupMenuButton<String>(onSelected: (v) {}, itemBuilder: (context) => const [PopupMenuItem(value: 'remove', child: Text('Remove'))]),
                ),
              )),
          const SizedBox(height: 16),
          FilledButton.icon(onPressed: _inviteMember, icon: const Icon(Icons.mail_outline), label: const Text('Invite Member')),
        ],
      ),
    );
  }
  ...
  void _inviteMember() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(context), child: const Text('Send Invite'))],
      ),
    );
  }
}
