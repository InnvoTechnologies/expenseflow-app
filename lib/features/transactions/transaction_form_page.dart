...
class _TransactionFormPageState extends State<TransactionFormPage> {
  ...
  void initState() {
    ...
    categoryId = widget.existing?.categoryId ?? DummyData.categories.firstWhere((c) => c.type == CategoryType.expense && c.parentId != null).id;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existing == null ? 'New Transaction' : 'Edit Transaction')),
      body: Form(
        key: formKey,
        child: ListView(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              value: categoryId,
              items: DummyData.categories.where((c) => type == TransactionType.income ? c.type == CategoryType.income : c.type == CategoryType.expense && c.parentId != null).map((c) => DropdownMenuItem(value: c.id, child: Text(c.name))).toList(),
              onChanged: (v) => setState(() => categoryId = v),
            ),
            const SizedBox(height: 8),
            DropdownButtonFormField<String>(
              value: payeeId,
              items: DummyData.payees.map((p) => DropdownMenuItem(value: p.id, child: Text(p.name))).toList(),
            ...
            FilledButton(
              onPressed: () {
                final t = Transaction(id: widget.existing?.id ?? 'tx${DateTime.now().millisecondsSinceEpoch}', amount: amount, type: type, date: date, description: description.isEmpty ? (type == TransactionType.income ? 'Income' : 'Expense') : description, accountId: accountId, toAccountId: null, categoryId: categoryId, payeeId: payeeId, status: 'POSTED');
