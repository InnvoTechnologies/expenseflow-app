  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBarWidget(title: 'Transactions', actions: [
        ...
        IconButton(onPressed: () async {
          final created = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => const TransferFormPage()));
          if (created is Transaction) setState(() => items.insert(0, created));
      ...
      body: ListView(
        ...
        children: [
          FilterTabs(index: filter, labels: const ['All', 'Income', 'Expense', 'Transfer'], onChanged: (i) => setState(() => filter = i)),
          const SizedBox(height: 8),
          ...filtered.map((t) => Card(
                child: ListTile(
                  leading: CircleAvatar(child: Icon(t.type == TransactionType.income ? Icons.arrow_downward : t.type == TransactionType.expense ? Icons.arrow_upward : Icons.swap_horiz)),
                  title: Text(t.description),
                  subtitle: Text('${t.date.year}-${t.date.month.toString().padLeft(2, '0')}-${t.date.day.toString().padLeft(2, '0')} • ${t.status}'),
                  ...
                  onTap: () async {
                    final updated = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => TransactionFormPage(existing: t)));
